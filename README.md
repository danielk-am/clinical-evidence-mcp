...
**Search public medical literature, trial registrations, drug labels, and adverse event reports from any compatible MCP client.**

Clinical Evidence MCP is a read-only, source-first Model Context Protocol server. It queries official public APIs at request time and returns structured records with stable identifiers, direct source URLs, and retrieval timestamps.

Hosted endpoint: `https://clinical-evidence-mcp.danielk.am/mcp`

A bearer token is required. Tokens are issued by the operator and bound to one email account. Tokens are never accepted in URLs.
...
The server also exposes `clinical-evidence://sources`, a resource describing the upstream datasets and their limits.
  ...
  clinical-evidence:
    ...
    customUserVars:
      CLINICAL_EVIDENCE_TOKEN:
        ...
        description: Enter the personal token issued by the server operator.
        sensitive: true
```

Restart LibreChat, save the token under MCP Settings, then reinitialise the connector. Each LibreChat user can have a separate token.

## Connect Claude

Claude's request-header authentication is currently a beta feature with staged availability. If it is enabled for your account:
...
If the Request headers section is absent, use LibreChat or another bearer-header client. Per-user Claude sign-in requires an OAuth 2.1 authorization service, which is outside this release.
...
Test an initialized server from a terminal:
...
The registry stores SHA-256 token digests, immutable account IDs, normalized email addresses, labels, and revocation timestamps in `/data/tokens.json`. The raw secret is not stored. Protect `/data`, backups, and terminal output as sensitive operator data.
...
| `UPSTREAM_TIMEOUT_MS` | `20000` | Provider request timeout |
